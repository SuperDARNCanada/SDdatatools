...
from utils import file_exists, check_rst_command
...
class ConvectionMaps():
    ...
    def arguement_parser(self, arguements):
        ...
        if not os.path.exists(self.parameter['datapath']):
            raise PathDoesNotEcistException(self.parameter['datapath'])
    ...
    def _generate_paths(self):
        """
        generate the folder paths for the various data
        """
    ...
    def _generate_radar_grid_file(self, radar_abbrv, radar_ext):
        ...
        if 'C.fit' in data_filename:
            try:
                data_file, radar_abbrv = self.convert_fit_to_fitacf(data_path)
